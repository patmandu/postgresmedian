...
/*
 * Pat Mancuso - Timescale homework project 12/2018 Find the median value
 */
...
median_transfn(PG_FUNCTION_ARGS) {
	...
	if (!AggCheckCallContext(fcinfo, &agg_context)) {
		...
		if (!state) {
			/*
			 * If state is null, this is first call, which means
			 * we should create the state
			 */
			state = palloc(sizeof(MedianState));
			if (!state) {
				MemoryContextSwitchTo(old_context);
				elog(ERROR, "median_transfn failed to alloc state");
			}

			memset(state, 0, sizeof(MedianState));
			state->values_num = 0;

			{
				...
				int		workMem = 65536;	/* wild guess - should
									 * be tuned */
				bool		randomAccess = false;
				char	       *oprname = "<";

				/*
				 * Get the sort operator oid (ref:
				 * namespace.c)
				 */
		...
		/*
		 * Now that we've got a state and a tuplesort state, and it's
		 * ready to accept the data...store it...if it's not null
		 */
		if (PG_ARGISNULL(1)) {
...
median_finalfn(PG_FUNCTION_ARGS) {
	...
	if (!AggCheckCallContext(fcinfo, &agg_context)) {
		...
		if (!state) {
			/*
			 * State will be null if the table was empty...all
			 * done
			 */
			...
			/*
			 * cheat a bit here, use the mid value even if it's
			 * an even number of total values to avoid trying to
			 * average non-numeric values
			 */
